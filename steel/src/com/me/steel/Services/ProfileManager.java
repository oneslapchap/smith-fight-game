...
public class ProfileManager
{
    ...
    public Profile retrieveProfile()
    {
        ...
        Gdx.app.log( Steel.LOG, "Retrieving profile from: " + profileDataFile.path() );
        ...
        if( profileDataFile.exists() ) {
            ...
            try {
                ...
                if( profileAsText.matches( "^[A-Za-z0-9/+=]+$" ) ) {
                    Gdx.app.log( Steel.LOG, "Persisted profile is base64 encoded" );
                ...
                // log the exception
                Gdx.app.error( Steel.LOG, "Unable to parse existing profile data file", e );
    ...
    protected void persist(Profile profile) {
        // create the handle for the profile data file
        FileHandle profileDataFile = Gdx.files.local( PROFILE_DATA_FILE );
        Gdx.app.log( Steel.LOG, "Persisting profile in: " + profileDataFile.path() );
