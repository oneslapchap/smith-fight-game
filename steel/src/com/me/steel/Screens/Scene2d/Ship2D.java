...
import com.me.steel.Domain.Ship;
import com.me.steel.Utils.VectorUtils;
...
public class Ship2D extends Image {
	...
	private Ship2D(Ship ship, Array<AtlasRegion> tiltAnimationFrames) {
		...
		// Changed from false to Touchable.disabled
		...
		for (AtlasRegion region : tiltAnimationFrames) {
			tiltAnimationDrawables.put(region,
	...
	private void tiltShip(float delta) {
		...
		if (velocity.x < 0) {
			frame = tiltAnimation.getKeyFrame(tiltAnimationStateTime += delta,
					false);
